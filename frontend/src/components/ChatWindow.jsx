...
export default function ChatWindow({ role, patientId, patientName, user }) {
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      // Instantly update the chat without waiting for socket emit
    ...
    <Box mt={4}>
      ...
      <Select
        ...
        onChange={(e) => {
          ...
          const selectedUser = appointments.find((apt) =>
            role === 'doctor' ? apt.patientId?._id === selectedUserId : apt.doctorId?._id === selectedUserId
          );
          setSelectedChatUser(role === 'doctor' ? selectedUser?.patientId : selectedUser?.doctorId);
        }}
      >
        ...
        {appointments.map((apt) => {
          ...
          return (
            <MenuItem key={apt._id} value={userObj?._id}>
              {userObj?.name}
            </MenuItem>
          );
        })}
      ...
      <Paper
        elevation={3}
        sx={{ my: 2, p: 2, height: '300px', overflowY: 'auto' }}
        ref={chatBoxRef}
      >
        {chat.length === 0 ? (
